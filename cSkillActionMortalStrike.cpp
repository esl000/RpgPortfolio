...
cSkillActionMortalStrike::cSkillActionMortalStrike()
		: m_pOwnerMesh(NULL)
		, m_pTarget(NULL)
		, m_pEffect(NULL)
		, m_bIsHit(true)
		, m_dwMortalStrikeAnim(0)
		, m_bSoundCheck(false)
{
}
...
void cSkillActionMortalStrike::Start()
{
	if (m_pOwner)
	{
		...
		m_pEffect->SetFaceColor(D3DXCOLOR(0.0f, 0.0f, 255.0f, 1.0f));
...
void cSkillActionMortalStrike::Update()
{
	...
	if (m_pOwner->GetAnimChange() && m_pOwnerMesh && m_pOwnerMesh->GetCurrentAni() != m_dwMortalStrikeAnim)
		m_pOwnerMesh->SetAnimationIndex(m_dwMortalStrikeAnim);
	...
	if (dPos / dPeriod > 0.6f)
	{
		...
		{
			//cout << "죽격!!!" << endl;
			m_pTarget->Damage(m_pOwner->GetCurrentDamage() * 2.f);
			// 치유력감소효과인데... 아마 쓸모 없을듯 데미지랑 애니메이션만 수정하면될듯
			m_bIsHit = false;
