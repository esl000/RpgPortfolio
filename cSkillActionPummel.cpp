...
cSkillActionPummel::cSkillActionPummel()
		: m_pOwnerMesh(NULL)
		, m_pTarget(NULL)
		, m_pEffect(NULL)
		, m_bIsHit(true)
...
void cSkillActionPummel::Start()
{
	if (m_pOwner)
	{
		...
		m_pEffect->SetFaceColor(D3DXCOLOR(0.0f, 1.0f, 1.0f, 1.0f));
...
void cSkillActionPummel::Update()
{
	...
	if (m_pOwner->GetAnimChange() && m_pOwnerMesh && m_pOwnerMesh->GetCurrentAni() != m_dwPummelAnim)
		m_pOwnerMesh->SetAnimationIndex(m_dwPummelAnim);
	...
	if (!m_bSoundCheck)
	{
		...
		g_pSoundManager->play("FallingDamage", 1.0f);
		m_bSoundCheck = true;
	}
